...
function localInjection() {
    ...
    function getProjFullPath(req) {
        var file = path.join(req.staticSource,  "index.html");
        //console.log('project file path = ', file);
        return fs.existsSync(file) ? file : null;
    }
    function handle(req, res, next) {
        // TODO: DRY (see proxyRemote function)
        if (req.query.enableripple) {
            var xxx = path.join(HOSTED_PKG_DIR, "index.html");
            console.log('hosted pkg dir = ', xxx);
            res.sendfile(path.join(HOSTED_PKG_DIR, "index.html"), function (err) {
                if (err) {
                    console.error(err);
                } else {
                    console.log('sended file = ', xxx);
                }
            });
        ...
        } else if (req.path.match(/^\/emulate\/.*\/$/)) {
            var fullPath = getProjFullPath(req);
            console.log('fullpath2 = ', fullPath);
            if (fullPath) {
                inject(fullPath, req, res);
            } else {
                next();
    ...
    return function (req, res, next) {
        //console.log('------------- hosted.js called ---------------');
        //console.log('static Platform : ', req.staticPlatform);
        //console.log('static Source : ', req.staticSource);
