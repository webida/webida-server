        ...
        if (error) {
            ...
            if (stat.isDirectory()) {
                flinkMap.updateLinkByExec(fsid, localPath, function (err) {
                    if (!err) {
                        logger.info('UIP: updateByExec done');
                    }
                    if (cb) {
                        cb();
                    }
