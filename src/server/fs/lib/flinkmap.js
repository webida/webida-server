...
//<!-- uip-meta-fileid:20140708-309f-4bb0-9fe8-bbea65166b6b -->
//CSS 파일의 파일 아이디
///* uip-meta-fileid:20140703-404c-4e72-a74d-55e5e523cef9 */
...
function dbUpdate(fsid, fileid, filepath, cb) {
    ...
    db.flink.update(query, update, { upsert: true }, function (err, flinkInfo) {
        if (err) {
            logger.error('UIP:failed to update filelink - ', update, err);
            return cb(new Error('UIP:failed to update file link from database:' + err.toString()));
        }
        if (cb) {
            cb(null, flinkInfo);
        }
...
function dbInsertByPath(fsid, newFileId, newPath, cb) {
    ...
    db.flink.insert(insertDoc, function (err, flinkInfo) {
        if (err) {
            logger.error('UIP:failed to update filelink', insertDoc, err);
            return cb(new Error('UIP:failed to update file link from database:' + err.toString()));
        }
        if (cb) {
            cb(null);
...
var dbRemoveByPath = function (fsid, filepath, cb) {
    ...
    try {
        db.flink.remove(flinkInfo, function (err) {
            if (err) {
                //logger.error('UIP:failed to remove file link from database', flinkInfo, err);
                return cb(new Error('UIP:failed to remove file link from database:' + err.toString()));
            }
            cb(null, flinkInfo);
        });
    } catch (e) {
    ...
    listDir(dirPath, function (err, fileList) {
        ...
        function recursiveRemove(callback) {
            ...
            dbRemoveByPath(fsid, filePath, function (err, flinkInfo) {
                if (!err) {
                    //logger.debug('UIP:removed - ', flinkInfo);
                }                       
                taskCount++;
            ...
            listDir(dirNewPath, function (err, newFileList) {
                if (err) {
                    ...
                    recursiveUpdate(function (err) {
                        logger.info('---------UIP: ------ recursive copy work for link is done');
                        return cb(err);
                    });
                }
            });
...
var updateFileLink = function(fsid, filePath, cb) {
    ...
    if (line) {
        var fileid = parseFileId(line);
        if (!fileid) {
            cb(new Error('UIP:failed to get file id'));
        } else {
            dbUpdate(fsid, fileid, filePath, cb); 
        }
    } else {
        cb(new Error('UIP:failed to read file id'));
    }
};
