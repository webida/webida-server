...
exports.createUIPAdmin = function (callback) {
    async.waterfall([
        function(next) {
            userdb.findUser({email: config.services.auth.uipAdmin.email}, function(err, results) {
                if (err)
                    return next(new ServerError('Creating the uipAdmin account failed.'));
            ...
            userdb.addUser(config.services.auth.uipAdmin, function(err, user) {
                if (err){
                    logger.error(err);
                    return next(new ServerError('Creating the uipAdmin account failed.'));
                } else {
...
router.get('/login_uip',
    function (req, res) {
        res.render('login_uip');
    }
);
...
router.post('/webida/api/oauth/uipsignup', function(req, res) {
    multipartMiddleware,
    sqlConn.beginTransaction(function (err) {
        if (err)
            return next(err);
        ...
        logger.info('uipSignup', authInfoArr);
        ...
        async.eachSeries(authInfoArr, function(authInfo, cb) {
            ...
            if (err) {
                ...
                sqlConn.commit(function (err) {
                    if (err) {
                        sqlConn.rollback(function() {
                            return res.sendfail('uipSignup failed(server internal error)');
                        });
                    }
...
router.post('/webida/api/oauth/uipupdateuser',
    multipartMiddleware,
    userdb.verifyToken,
    updateUser
);
